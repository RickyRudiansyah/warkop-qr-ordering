...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchTable = async () => {
      try {
        ...
        const res = await api.get<Table>(`/tables/${tableNumber}`)
        setTable(res.data)
      } catch {
  ...
  const handleSubmit = async () => {
    ...
    if (!table?.id) {
      toast.error('Data meja belum siap')
      return
    }
  ...
  return (
    <div className="min-h-dvh bg-[var(--color-surface)] flex flex-col">
      <div className="glass sticky top-0 z-10 px-4 py-4 flex items-center gap-3">
        <button
          onClick={() => step > 0 ? setStep(s => s - 1) : navigate(`/order?table=${tableNumber}`)}
      ...
      <div className="flex items-center px-6 py-4 gap-2">
        {STEPS.map((label, i) => (
          <div key={i} className="flex items-center gap-2 flex-1 last:flex-none">
            ...
            <span className={`text-xs font-medium ${i === step ? 'text-[var(--color-text)]' : 'text-[var(--color-text-muted)]'}`}>
              {label}
            </span>
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-px ${i < step ? 'bg-[var(--color-primary)]' : 'bg-[var(--color-border)]'}`} />
            )}
          </div>
        ))}
      </div>
        ...
        {step === 0 && (
          <div className="space-y-3">
            <h2 className="font-bold text-base mb-4">
              Review Pesanan — {table?.label || `Meja ${tableNumber}`}
            </h2>
      ...
      {step < 2 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 glass border-t border-[var(--color-border)]">
          ...
          {step === 1 && (
            <Button onClick={handleSubmit} disabled={!payment || loading || !table?.id} className="w-full" size="lg">
